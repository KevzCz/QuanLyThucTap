...
interface Props {
  ...
  onSuccess: (registration: TeacherRegistration) => void;
}
...
const RegisterSubjectDialog: React.FC<Props> = ({ open, onClose, subject, onSuccess }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const registration: TeacherRegistration = {
        teacherId: "current-user", // This will be handled by server
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Xác nhận tham gia giảng dạy"
      widthClass="max-w-md"
      actions={
        <div className="flex gap-3">
          ...
          >
            {isLoading ? "Đang đăng ký..." : "Xác nhận tham gia"}
          </button>
        </div>
      }
    >
      <div className="space-y-4">
        <div className="text-center">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            Tham gia giảng dạy môn thực tập
          </h3>
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-gray-500">Sinh viên hiện tại:</span>
            <span className="font-medium">{subject.currentStudents}/{subject.maxStudents}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Giảng viên tham gia:</span>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-amber-800">
              ...
              <ul className="space-y-1 text-xs">
                <li>• Bạn tham gia với tư cách <strong>giảng viên hướng dẫn</strong>, không chiếm slot sinh viên</li>
                <li>• Ban chủ nhiệm sẽ phân công sinh viên cho bạn theo năng lực và số lượng</li>
                <li>• Bạn có thể rời khỏi môn thực tập thông qua Ban chủ nhiệm</li>
                <li>• Mỗi giảng viên chỉ được tham gia một môn thực tập trong một kỳ</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-gray-700">
            Tôi đã đọc và hiểu các yêu cầu, đồng ý tham gia giảng dạy môn thực tập này
          </span>
        </label>
      </div>
    </Modal>
